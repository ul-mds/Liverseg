...
def main(cfg:DictConfig):
    ...
    trainloaders, validationloaders, testloader = prepare_multiple_datasets(cfg.dataset) 
    ...
    client_fn = generate_client_fn(trainloaders, validationloaders, cfg, device) 
           
    ## 5. Define the federated strategy
    strategy = instantiate(cfg.strategy,
                           evaluate_fn=get_evaluate_fn(cfg, testloader, device)
                           )
